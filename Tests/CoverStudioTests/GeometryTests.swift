...
final class GeometryTests: XCTestCase {
  ...
  // KDP hardcover layout is [wrap][back][spine][front][wrap] — panels are adjacent
  // to the spine; the hinge is a fold inside each panel, not an extra strip.
  // The panel chain must tile the canvas: wrap + back + spine + front + wrap
  // sums to the full width (±1px rounding), and no hinge strips shift the panels.
  func testHardcoverPanelsAreAdjacentToSpine() throws {
    var data = CoverData()
    data.bindingType = .hc
    ...
    XCTAssertEqual(g.backLeft, CoverGeometry.px(0.591))
    XCTAssertEqual(g.spineLeft, g.backLeft + g.frontWidth)
    XCTAssertEqual(g.frontLeft, g.spineRight)
    // No gap or overlap between panels and spine, either side.
    XCTAssertEqual(g.spineLeft - g.backRight, 0)
    XCTAssertEqual(g.frontLeft - g.spineRight, 0)
    // The chain [wrap][back][spine][front][wrap] fills the full canvas.
    XCTAssertEqual(
      g.wrap + g.frontWidth + g.spineWidth + g.frontWidth + g.wrap,
      g.totalWidth,
      accuracy: 1)
    // The spine sits centered between the two panel edges (no hinge shift).
    XCTAssertEqual(
      Double(g.spineLeft + g.spineRight) / 2, Double(g.totalWidth) / 2, accuracy: 1.5)
  }

  // KDP hardcover spine = page block (pages × paper thickness) + 0.189" board
  // allowance. Verified against KDP's calculator (100 pages B&W white → 0.4142").
  func testHardcoverSpineFormulaMatchesKDPCalculator() {
    XCTAssertEqual(
      HCDefaults(trimWidth: 6, trimHeight: 9).spineWidth(for: 100),
      100 * 0.002252 + 0.189,
      accuracy: 0.0001)
    XCTAssertEqual(
      HCDefaults(trimWidth: 6, trimHeight: 9)
        .spineWidth(for: 100, interior: .blackWhite, paper: .cream),
      100 * 0.0025 + 0.189,
      accuracy: 0.0001)
  }
  ...
  func testGeometryThrowsForZeroCustomTrimWidth() {
    ...
    XCTAssertThrowsError(try computeGeometry(from: data)) { error in
      guard let gErr = error as? GeometryError,
        case .customTrimMustBePositive = gErr
      else {
  ...
  func testGeometryThrowsForMissingHardcoverTemplate() {
    ...
    XCTAssertThrowsError(try computeGeometry(from: data)) { error in
      guard let gErr = error as? GeometryError,
        case .hardcoverMissingTemplate = gErr
      else {
