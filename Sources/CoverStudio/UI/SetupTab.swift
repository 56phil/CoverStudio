...
struct HCDefaults {
 ...
 /// page block (pages × paper thickness) + 0.189" board allowance.
 /// Verified against KDP's calculator: 100 pages B&W white → 0.4142".
 func spineWidth(
  for pageCount: Int, interior: InteriorType = .blackWhite, paper: PaperType = .white
 ) -> Double {
  Double(pageCount) * (spineMultipliers[interior]?[paper] ?? 0.002252) + 0.189
 }
 ...
 func fullWidth(
  ...
  frontWidth * 2 + spineWidth(for: pageCount, interior: interior, paper: paper) + wrapWidth * 2
 }
  ...
  Form {
   ...
   Section("Binding") {
    ...
    Picker("Paper", selection: $data.paperType) {
     ForEach(PaperType.options(for: data.interiorType, binding: data.bindingType), id: \.self) {
      Text($0.label).tag($0)
     }
    }
   }
   ...
   if data.bindingType == .hc {
    Section {
     Text(
      "Exact values from your printer's template calculator. Defaults shown are KDP estimates — verify with the calculator before submitting."
     )
     ...
     TemplateField(
      ...
      value: Binding(
       get: {
        data.templateFullCoverWidth
         ?? hcDefaults.fullWidth(
          for: data.resolvedPageCount(), interior: data.interiorType, paper: data.paperType)
       },
     ...
     TemplateField(
      "Full cover height",
      value: Binding(
       get: { data.templateFullCoverHeight ?? hcDefaults.fullHeight },
       set: { data.templateFullCoverHeight = $0 }))

     TemplateField(
      "Front cover width",
      value: Binding(
       get: { data.templateFrontCoverWidth ?? hcDefaults.frontWidth },
       set: { data.templateFrontCoverWidth = $0 }))

     TemplateField(
      "Front cover height",
      value: Binding(
       get: { data.templateFrontCoverHeight ?? hcDefaults.frontHeight },
     ...
     TemplateField(
      ...
      value: Binding(
       get: {
        data.templateSpineWidth
         ?? hcDefaults.spineWidth(
          for: data.resolvedPageCount(), interior: data.interiorType, paper: data.paperType)
       },
     ...
     TemplateField(
      "Hinge width",
      value: Binding(
       get: { data.templateHingeWidth ?? hcDefaults.hingeWidth },
       set: { data.templateHingeWidth = $0 }))

     TemplateField(
      "Wrap width",
      value: Binding(
       get: { data.templateWrapWidth ?? hcDefaults.wrapWidth },
