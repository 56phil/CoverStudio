...
struct CoverRenderer {
 ...
 func deriveSpineColor(from imagePath: String) -> NSColor {
  ...
  for i in stride(from: 0, to: thumbSize * thumbSize * 4, by: 4) {
   ...
   if let existing = seen[key] {
    seen[key] = (existing.count + 1, existing.color)
   } else {
    seen[key] = (1, (r, g, b))
   }
  }
  ...
  return NSColor(
   red: CGFloat(dr) / 255, green: CGFloat(dg) / 255, blue: CGFloat(db) / 255, alpha: 1)
 }
 ...
 func renderFullCover(includeGuides: Bool = true) -> CGImage? {
  let totalW = geometry.totalWidth
  let totalH = geometry.totalHeight
  guard
   let ctx = CGContext(
 ...
 private func drawBack(ctx: CGContext, spineColor: NSColor) {
  ...
  let backBox: CGRect =
   g.effectiveBackLeft < g.spineLeft
   ? CGRect(x: 0, y: 0, width: CGFloat(g.spineLeft), height: totalH)
   : CGRect(
    x: CGFloat(g.spineRight), y: 0, width: CGFloat(g.totalWidth - g.spineRight), height: totalH)
  ...
  let blurb = data.resolvedBlurb().trimmingCharacters(in: .whitespaces)
  var blurbPillowBottom =
   CGFloat(g.trimTop) + CGFloat(CoverGeometry.px(CoverLayoutDefaults.backBlurbTopInches))
  if !blurb.isEmpty {
   let bx = safeX + CGFloat(data.blurbOffsetXInches) * CGFloat(geometry.dpi)
   let byPillow =
    CGFloat(g.trimTop) + CGFloat(CoverGeometry.px(CoverLayoutDefaults.backBlurbTopInches))
    + CGFloat(data.blurbOffsetYInches) * CGFloat(geometry.dpi)
   let maxYPillow = CGFloat(g.trimBottom - g.safe) - 80
   let blurbMaxWidth =
    data.blurbWidthInches > 0
  ...
  if !quote.isEmpty {
   ...
   let quotePillowY =
    blurbPillowBottom + CGFloat(CoverGeometry.px(CoverLayoutDefaults.backQuoteGapInches)) + CGFloat(
     data.quoteOffsetYInches) * CGFloat(geometry.dpi)
   _ = drawCenteredText(
    ctx: ctx, text: "\u{201C}\(quote)\u{201D}", font: font, color: color, centerX: qcx,
    y: cgYf(quotePillowY), shadow: false)
   ...
   if !attr.isEmpty {
    ...
    let attributionPillowY =
     quotePillowY + CGFloat(CoverGeometry.px(CoverLayoutDefaults.backQuoteAttributionGapInches))
     + CGFloat(data.quoteAttributionOffsetYInches) * CGFloat(geometry.dpi)
    _ = drawCenteredText(
     ctx: ctx, text: atext, font: afont, color: acolor, centerX: ax, y: cgYf(attributionPillowY),
     shadow: false)
   }
  }
  ...
  let photoPillowY =
   CGFloat(g.backBarcodeZoneBottom) - photoSize + CGFloat(data.authorPhotoOffsetYInches)
   * CGFloat(geometry.dpi)
    ...
    case .circle, .square:
     ...
     if let cropped = cg.cropping(to: CGRect(x: sx, y: sy, width: CGFloat(sz), height: CGFloat(sz)))
     {
      ...
      if data.authorPhotoShape == .circle {
       let clip = CGPath(
        roundedRect: rect, cornerWidth: photoSize / 2, cornerHeight: photoSize / 2, transform: nil)
       ctx.addPath(clip)
       ctx.clip()
      }
  ...
  if !bio.isEmpty {
   ...
   let bioStartPillow =
    CGFloat(g.trimBottom) - CGFloat(CoverGeometry.px(CoverLayoutDefaults.backAuthorBioBottomInches))
    + CGFloat(data.authorBioOffsetYInches) * CGFloat(geometry.dpi)
 ...
 private func drawFront(ctx: CGContext) {
  ...
  if !titleText.isEmpty {
   ...
   let fontSize = fitFontSize(
    text: titleText, maxWidth: maxW,
    maxSize: Int(178 * scale), minSize: 14, fontName: fontName)
   let font = CTFontCreateWithName(fontName as CFString, fontSize, nil)
   let tx =
    data.resolvedTitleCenterX()
    ? centerX
    : centerX + CGFloat(data.resolvedOffsetX()) * CGFloat(geometry.dpi)
   let tyPillow =
    data.resolvedTitleCenterY()
    ? centerPillowY
    : CGFloat(g.trimTop) + CGFloat(CoverGeometry.px(CoverLayoutDefaults.frontTitleTopInches))
     + CGFloat(data.resolvedOffsetY()) * CGFloat(geometry.dpi)
   _ = drawCenteredText(
    ctx: ctx, text: titleText, font: font, color: gold, centerX: tx, y: cgYf(tyPillow), shadow: true
   )
   ...
   if !subtitle.isEmpty {
    ...
    let sfont = CTFontCreateWithName(
     "Arial" as CFString,
     fitFontSize(
      text: subtitle, maxWidth: maxW, maxSize: maxSubtitleSize, minSize: 10, fontName: "Arial"), nil
    )
    let scolor = NSColor(hex: data.colorAccent)
    let sx =
     data.resolvedSubtitleCenterX()
     ? centerX
     : centerX + CGFloat(data.resolvedSubtitleOffsetX()) * CGFloat(geometry.dpi)
    let syPillow =
     data.resolvedSubtitleCenterY()
     ? centerPillowY
     : tyPillow + CGFloat(CoverGeometry.px(CoverLayoutDefaults.frontSubtitleGapInches)) + CGFloat(
      data.resolvedSubtitleOffsetY()) * CGFloat(geometry.dpi)
    _ = drawCenteredText(
     ctx: ctx, text: subtitle, font: sfont, color: scolor, centerX: sx, y: cgYf(syPillow),
     shadow: false)
   }
  }
  ...
  if !authorText.isEmpty {
   ...
   let fontSize = fitFontSize(
    text: authorText, maxWidth: maxW,
    maxSize: Int(48 * scale), minSize: 12, fontName: fontName)
   let font = CTFontCreateWithName(fontName as CFString, fontSize, nil)
   let ax =
    data.resolvedAuthorCenterX()
    ? centerX
    : centerX + CGFloat(data.resolvedAuthorOffsetX()) * CGFloat(geometry.dpi)
   let ayPillow =
    data.resolvedAuthorCenterY()
    ? centerPillowY
    : CGFloat(g.trimBottom) - CGFloat(CoverGeometry.px(CoverLayoutDefaults.frontAuthorBottomInches))
     + CGFloat(data.resolvedAuthorOffsetY()) * CGFloat(geometry.dpi)
   _ = drawCenteredText(
    ctx: ctx, text: authorText, font: font, color: gold, centerX: ax, y: cgYf(ayPillow),
    shadow: false)
  }
  ctx.restoreGState()
 }
 ...
 private func drawSpine(ctx: CGContext, spineColor: NSColor) {
  ...
  if !st.isEmpty {
   let font = CTFontCreateWithName(
    (data.fontBold.isEmpty ? "Arial" : data.fontBold) as CFString, 42, nil)
   let tyPillow =
    CGFloat(g.trimTop) + CGFloat(CoverGeometry.px(CoverLayoutDefaults.spineTitleTopInches))
    + CGFloat(data.spineTitleOffsetYInches) * CGFloat(geometry.dpi)
   drawRotatedText(
    ctx: ctx, text: st, font: font, color: gold,
  ...
  if !sa.isEmpty {
   let font = CTFontCreateWithName(
    (data.fontRegular.isEmpty ? "Arial" : data.fontRegular) as CFString, 28, nil)
   let ayPillow =
    CGFloat(g.trimBottom) - CGFloat(CoverGeometry.px(CoverLayoutDefaults.spineAuthorBottomInches))
    + CGFloat(data.spineAuthorOffsetYInches) * CGFloat(geometry.dpi)
   drawRotatedText(
    ctx: ctx, text: sa, font: font, color: gold,
 ...
 private func drawGuides(ctx: CGContext) {
  ...
  for x in [
   CGFloat(g.effectiveBackLeft), CGFloat(g.effectiveBackRight),
   ...
   ctx.move(to: CGPoint(x: xp, y: 0))
   ctx.addLine(to: CGPoint(x: xp, y: totalH))
  }
  // Horizontal: Pillow trimTop → CG y = totalH - trimTop
  let trimTopCG = cgY(g.trimTop)
  let trimBottomCG = cgY(g.trimBottom)
  ctx.move(to: CGPoint(x: 0, y: trimTopCG))
  ctx.addLine(to: CGPoint(x: CGFloat(g.totalWidth), y: trimTopCG))
  ctx.move(to: CGPoint(x: 0, y: trimBottomCG))
  ctx.addLine(to: CGPoint(x: CGFloat(g.totalWidth), y: trimBottomCG))
  ctx.strokePath()
 }
 ...
 func drawCenteredText(
  ctx: CGContext, text: String, font: CTFont, color: NSColor,
  centerX: CGFloat, y: CGFloat, shadow: Bool
 ) -> CGFloat {
 ...
 private func fitFontSize(
  text: String, maxWidth: CGFloat, maxSize: Int, minSize: Int, fontName: String
 ) -> CGFloat {
