...
struct CoverGeometry {
  ...
  // KDP hardcover layout: [wrap][back][spine][front][wrap]. The hinge is a fold
  // INSIDE each cover panel, not an extra strip, so panels are adjacent to the spine.
  var spineLeft: Int { backRight }
  var spineRight: Int { spineLeft + spineWidth }
  var frontLeft: Int { spineRight }
  ...
  init(
    data: CoverData, totalWidthInches: Double, totalHeightInches: Double, frontWidthInches: Double,
    frontHeightInches: Double, spineInches: Double, bleedInches: Double, hingeInches: Double,
    wrapInches: Double, safeInches: Double
  ) {
...
func computeGeometry(from data: CoverData) throws -> CoverGeometry {
  ...
  if data.bindingType == .pb {
    ...
    guard let tfw = data.templateFullCoverWidth,
      ...
      let tww = data.templateWrapWidth
    else {
