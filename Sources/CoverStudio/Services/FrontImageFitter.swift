...
enum FrontImageFitter {
 ...
 private static func fittedModified(after fitted: URL, source: String) -> Bool {
  guard let fittedAttrs = try? FileManager.default.attributesOfItem(atPath: fitted.path),
   ...
   let fittedDate = (fittedAttrs[.creationDate] as? Date)
    ?? (fittedAttrs[.modificationDate] as? Date),
 ...
 static func updateFittedImage(
  ...
  if FileManager.default.fileExists(atPath: fitted.path),
   let fittedImage = NSImage(contentsOfFile: fitted.path)?.cgImage(
  ...
  if mode == .stretch {
   ...
   // Contain: whole source visible, centered on a black letterbox.
   let scale = min(
    CGFloat(panelWidth) / CGFloat(cg.width),
    CGFloat(panelHeight) / CGFloat(cg.height))
   let sw = CGFloat(cg.width) * scale
   let sh = CGFloat(cg.height) * scale
   ctx.draw(
    cg,
    in: CGRect(
     x: (CGFloat(panelWidth) - sw) / 2,
     y: (CGFloat(panelHeight) - sh) / 2,
     width: sw,
     height: sh))
  } else {
   let scale = max(
    CGFloat(panelWidth) / CGFloat(cg.width), CGFloat(panelHeight) / CGFloat(cg.height))
   let sw = CGFloat(cg.width) * scale
   let sh = CGFloat(cg.height) * scale
   ctx.draw(
    cg,
    in: CGRect(
     x: (CGFloat(panelWidth) - sw) / 2, y: (CGFloat(panelHeight) - sh) / 2, width: sw, height: sh))
  }

  guard let fittedImage = ctx.makeImage() else { return nil }
  guard
   let dest = CGImageDestinationCreateWithURL(
    fitted as CFURL, UTType.png.identifier as CFString, 1, nil)
  else {
   return nil
  }
